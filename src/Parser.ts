...
export class Parser {
    ...
    parseMangaDetails ($: any, mangaId: string): Manga {
        ...
        tagSections[0]!.tags = entity.genre.map((elem: string) => createTag({ id: elem.toLowerCase(), label: elem }))
    ...
    parseSearch(data: any, metadata: any): MangaTile[] {
        ...
        for (const elem of directory) {
            ...
            if (!mType) mType = metadata.type.every((i: string) => elem.t?.toLowerCase()?.includes(i))
            if (!mScanStatus) mScanStatus = metadata.scanStatus.every((i: string) => elem.ss?.toLowerCase()?.includes(i))
            if (!mPublishStatus) mPublishStatus = metadata.publishStatus.every((i: string) => elem.ps?.toLowerCase()?.includes(i))
            if (!mTranslation) mTranslation = metadata.translation.every((i: string) => elem.o?.toLowerCase()?.includes(i))

            if (!mGenre) mGenre = metadata.genre.every((i: string) => elem.g?.map((genre: string) => genre.toLowerCase())?.includes(i))
            if (mGenreNo) mGenreNo = metadata.genreNo.every((i: string) => elem.g?.map((genre: string) => genre.toLowerCase())?.includes(i))
    ...
    parseSearchTags(data: any): TagSection[] {
        ...
        tagSections[0]!.tags = genres.map((tag: any) => createTag({ id: `genre_${tag.toLowerCase()}`, label: tag }))

        const typesHTML = data.match(/"Type"\s*: (.*),/g)?.[1]
        const types = JSON.parse(typesHTML.match(/(\[.*])/)?.[1].replace(/'/g, '"'))
        tagSections[1]!.tags = types.map((tag: any) => createTag({ id: `type_${tag.toLowerCase()}`, label: tag }))

        const scanStatusHTML = data.match(/"ScanStatus"\s*: (.*),/g)?.[1]
        const scanStatus = JSON.parse(scanStatusHTML.match(/(\[.*])/)?.[1].replace(/'/g, '"'))
        tagSections[2]!.tags = scanStatus.map((tag: any) => createTag({ id: `scan_status_${tag.toLowerCase()}`, label: tag }))

        const publishStatusHTML = data.match(/"PublishStatus"\s*: (.*),/g)?.[1]
        const publishStatus = JSON.parse(publishStatusHTML.match(/(\[.*])/)?.[1].replace(/'/g, '"'))
        tagSections[3]!.tags = publishStatus.map((tag: any) => createTag({ id: `publish_status_${tag.toLowerCase()}`, label: tag }))
    ...
    parseHomeSections($: any, data: any, sectionCallback: (section: HomeSection) => void): void {
        const topTenSection = createHomeSection({ id: 'top_ten', title: 'Top Ten', type: HomeSectionType.featured })
    ...
    parseViewMore(data: any, homepageSectionId: string): MangaTile[] {
        const manga: MangaTile[] = []
        const mangaIds: Set<string> = new Set<string>()

        if (!regex[homepageSectionId]) createPagedResults({results: []})
